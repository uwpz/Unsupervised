...
def prec_rec_auc(y_true, y_pred):
    if y_pred.ndim == 2:
        if y_pred.shape[1] == 2:
            y_pred = y_pred[:, 1]
...
d_scoring = {"CLASS": {"auc": make_scorer(auc, greater_is_better = True, needs_proba = True),
                       "prec_rec_auc": make_scorer(prec_rec_auc, greater_is_better = True, needs_proba = True),
...
def plot_corr(df, features, cate_corr_type = "contingency", cutoff = 0, n_cluster = 5, w = 8, h = 6, pdf = None):
    ...
    if len(metr):
        df_corr = abs(df[metr].corr(method = "pearson"))
        d_new_names = dict(zip(df_corr.columns.values,
    ...
    if len(metr):
        ax_act.set_title("Absolute pearson correlation (cutoff at " + str(cutoff) + ")")
    if len(cate):
        if cate_corr_type == "contingency":
